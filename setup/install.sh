...
if [ ! -f "$REPO_DIR/SKILL.md" ]; then
    echo "❌ Not in the mur-core directory"
    exit 1
fi
...
def replace_paths(obj):
    if isinstance(obj, str):
        obj = obj.replace('~/clawd/skills/mur-core', '$REPO_DIR')
        obj = re.sub(r'\\\$HOME/clawd/skills/mur-core', '$REPO_DIR', obj)
        return obj
...
# Check if any hook command references mur-core
import json as j
found = False
for event, groups in hooks.items():
    for group in (groups if isinstance(groups, list) else []):
        for hook in group.get('hooks', []):
            if 'mur-core' in hook.get('command', ''):
                found = True
    ...
    if event not in existing_hooks:
        ...
        for matcher_group in event_hooks:
            for hook in matcher_group.get('hooks', []):
                ...
                if 'mur-core' in cmd and cmd not in existing_commands:
                    existing_hooks[event].append(matcher_group)
                    break
                elif 'mur-core' not in cmd:
                    existing_hooks[event].append(matcher_group)
                    break
...
if crontab -l 2>/dev/null | grep -q "mur-core.*auto_learn"; then
    echo ""
    ...
    if [[ "${SETUP_LEARN:-N}" =~ ^[Yy] ]]; then
        (crontab -l 2>/dev/null | grep -v "mur-core.*auto_learn"; echo "0 3 * * * $CRON_CMD_LEARN") | crontab -
...
if [ "$MODE" = "team" ]; then
    ...
    if crontab -l 2>/dev/null | grep -q "mur-core.*auto-commit"; then
        echo "  ✅ Auto-push cron already set — skipping"
    else
        echo ""
        read -p "  Set up auto-push cron (every 30 min)? [y/N]: " SETUP_PUSH
        if [[ "${SETUP_PUSH:-N}" =~ ^[Yy] ]]; then
            (crontab -l 2>/dev/null | grep -v "mur-core.*auto-commit"; echo "*/30 * * * * $CRON_CMD_PUSH") | crontab -
    ...
    if crontab -l 2>/dev/null | grep -q "mur-core.*sync_to_claude_code"; then
        echo "  ✅ Auto-pull cron already set — skipping"
    else
        read -p "  Set up auto-pull cron (every hour)? [y/N]: " SETUP_PULL
        if [[ "${SETUP_PULL:-N}" =~ ^[Yy] ]]; then
            (crontab -l 2>/dev/null | grep -v "mur-core.*sync_to_claude_code"; echo "0 * * * * $CRON_CMD_PULL") | crontab -
