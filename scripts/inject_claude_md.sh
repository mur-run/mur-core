#!/usr/bin/env bash
# inject_claude_md.sh — Inject learned rules into a project's CLAUDE.md
# Usage:
...
# --- Markers ---
START_MARKER="<!-- LEARNED RULES (auto-generated by murmur-ai) -->"
...
with open('$CLAUDE_MD', 'w') as f:
    f.writelines(out)
"
  echo "✅ Removed learned rules section from $CLAUDE_MD" >&2
  exit 0
fi
...
if [[ ${#HIGH_PATTERNS[@]} -eq 0 ]]; then
  echo "📭 No HIGH confidence patterns found. Nothing to inject." >&2; exit 0
fi
...
# --- Dry run ---
if [[ "$DRY_RUN" == "true" ]]; then
  echo "🔍 [DRY RUN] Would inject ${#HIGH_PATTERNS[@]} rule(s) into $CLAUDE_MD:" >&2
  echo "" >&2
  echo "$INJECTION"
  exit 0
fi
...
if [[ ! -f "$CLAUDE_MD" ]]; then
  ...
  echo "✅ Created $CLAUDE_MD with ${#HIGH_PATTERNS[@]} learned rule(s)." >&2
  exit 0
fi
...
with open('$CLAUDE_MD', 'w') as f:
    f.write(content)
"
  echo "✅ Updated learned rules in $CLAUDE_MD (${#HIGH_PATTERNS[@]} rule(s))." >&2
else
  echo "" >> "$CLAUDE_MD"
  echo "$INJECTION" >> "$CLAUDE_MD"
  echo "✅ Appended ${#HIGH_PATTERNS[@]} learned rule(s) to $CLAUDE_MD." >&2
fi
